...
def train_epoch(model, train_loader, optimizer, criterion, device, metrics_tracker):
    ...
    with tqdm(total=len(train_loader.dataset), ncols=100, colour='#3eedc4') as t:
        ...
        #     loss = criterion(outputs[valid_mask], x_data[valid_mask])
        #     loss.backward()
        #     optimizer.step()

        #     metrics_tracker.update(outputs, x_data, valid_mask)
        #     train_loss += loss.item()
        #     t.set_postfix(loss=loss.item())
...
def main():
    config = load_config(config_path="/home/ubuntu/project/sentinel-2-ai-compressor/src/cfg/config_zarr.yaml")
